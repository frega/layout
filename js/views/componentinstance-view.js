  ...
  Drupal.layout.ComponentInstanceView = Backbone.View.extend({
    ...
    render:function () {
      ...
      this.setElement(Drupal.theme('layoutComponent', this.model.get('id'), this.model.get('label'), {
        'configurePath': '/admin/structure/layout/manage/' + drupalSettings.layout.id + '/blocks/' + this.model.get('container') + '/' + this.model.get('id') + '/edit',
        'deletePath': '/admin/structure/layout/manage/' + drupalSettings.layout.id + '/blocks/' + this.model.get('container') + '/' + this.model.get('id') + '/delete'
      }));
